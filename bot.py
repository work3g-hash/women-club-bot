...
ADMIN_ID = 691846456          # Telegram ID жены
...
async def get_rules(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Согласна — отправляем заявку администратору
    ...
    await query.edit_message_text(
        ...
        "Если есть вопросы — просто напиши нам!"
    )
    return ConversationHandler.END
...
async def handle_admin_decision(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data = query.data
    action, applicant_id = data.split("_", 1)
    ...
    if action == "approve":
        await context.bot.send_message(
            ...
            text=(
                ...
                f"Вот ссылка для вступления в нашу группу:\n{GROUP_LINK}\n\n"
                "После вступления бот представит тебя участницам 😊"
            ),
            parse_mode="Markdown"
        )
        await query.edit_message_text(
            query.message.text + "\n\n✅ *Заявка одобрена* — ссылка отправлена",
            parse_mode="Markdown"
        )
    else:
        await context.bot.send_message(
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Анкета отменена. Если захочешь попробовать снова — напиши /start 💛"
    )
    return ConversationHandler.END
